...
def dijkstra(graph, start_node, end_node):
    min_distances = {i: float("inf") for i in graph.nodes}
    node_queue = []
    min_distances[start_node] = 0
    # the start_node.value is used for heapq to sort,when there are two or  more than 2 item's first value are the same ,the node object can't to use the > to equal which one is bigger,so we store value for it
    heapq.heappush(node_queue, (0, start_node.value, start_node))
    while len(node_queue) >= 1:
        distance, start_node.value, current_node = heapq.heappop(node_queue)
        if current_node.edges is None:
            break
        for edge in current_node.edges:
            new_distance = distance + edge.distance
            if new_distance < min_distances[edge.node]:
                min_distances[edge.node] = new_distance
                heapq.heappush(node_queue, (new_distance,edge.node.value, edge.node))

    return min_distances[end_node]

#solution
import math  # Need math.inf constant


def dijkstra_solution(graph, start_node, end_node):
...
# print('Shortest Distance from {} to {} is {}'.format(node_A.value, node_C.value, graph.dijkstra(graph, node_A, node_C)))
