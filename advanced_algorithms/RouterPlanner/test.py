from math import sin, cos, sqrt, radians, asin, fabs
...
def shortest_path(M, start, goal):
    ...
    # 优先队列，存储 (估计距离，路径对象)
    priority_queue = []
    heappush(priority_queue, (0, Pot([start], 0, 0)))

    visited = set()

    while priority_queue:
        # 取出估计距离最小的路径
        _, selected_path = heappop(priority_queue)
        current_node = selected_path.arr[-1]
        current_distance = selected_path.distance
        ...
        # 遍历当前节点的邻居
        for neighbour in all_roads[current_node]:
            if neighbour in visited:
                continue

            # 计算新路径的距离和估计距离
            neighbour_distance = current_distance + get_distance_hav(
                all_intersections[current_node], all_intersections[neighbour]
            )
            heuristic = get_distance_hav(all_intersections[neighbour], goal_location)
            estimate_distance = neighbour_distance + heuristic

            # 将新路径加入优先队列
            new_path = Pot(selected_path.arr + [neighbour], neighbour_distance, estimate_distance)
            heappush(priority_queue, (estimate_distance, new_path))
...
def get_distance_hav(pot1, pot2):
    ...
    distance = 2 * 6378.137 * asin(sqrt(h))  # 地球半径：6378.137 km
    return distance
