...
print_integers(5)
"""Now let's consider what happens in the call 3_stacks_queues when print_integers(5) is called.

As expected, a frame will be created for the print_integers() function and pushed onto the call 3_stacks_queues.
...
The process keeps on like this until we hit the base case. When n <= 0, we return from the frame without calling the function print_integers() again. Because we have returned from the function call, the frame is discarded from the call 3_stacks_queues and the next frame resumes execution right after the line where we left off."""
